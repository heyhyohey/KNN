...
def cv_maN_value(cv, N):
    # min_period 옵션을 이용하여 할 수도 있음
    if N >= 5:
        return cv.rolling(window=N).mean()
    else:
        return "5일이 안되는 기간은 제외"


# 4. cv_maN_rate : 종가의 N일 이동평균의 일간 변화율
def cv_maN_rate(cv, N):
    if N >= 5:
        cv_ma_value = cv.rolling(window=N).mean()
        return (cv_ma_value[5] - cv_ma_value[4]) / cv_ma_value[4] * 100
    else:
        return "5일이 안되는 기간은 제외"

# 5. ud_Nd : (a) N일 연속 증가  1, (b) N일 연속 하락 -1, (c) 그렇지 않은 날 0
def ud_Nd(cvdv, N):
    increase_count, decrease_count, nothing_count = 0, 0, 0
    print(len(cvdv)) # 228
...
# csv 파일 읽어오기
csv_file = open('stock_history.csv', 'r', encoding='euc-kr')
stock_data = pd.read_csv(csv_file)

# 하나의 종목을 선택
one_stock = stock_data.loc[ stock_data["stockname"] == "GS글로벌"]
from collections import Counter
...
final_result.to_csv('C:/Users/black/Desktop/stock_history_added.csv', encoding='ms949')
print("Data가 성공적으로 추가됐습니다")
csv_file_read.close()close_value = one_stock["close_value"] # 종가

# 종가 일간 변화량
for i in range(len(close_value)-2):
    result = cv_diff_value(close_value[i], close_value[i+1])
    cv_amount.append(result)
print(cv_amount) # 228개의 data

result2 = cv_diff_rate(close_value[0], close_value[1])
result3 = cv_maN_value(close_value, 5)
result4 = cv_maN_rate(close_value, 5)
result5 = ud_Nd(cv_amount, 5)

print(round(result2, 2)) # 종가 일간 변화율 ( 소수점 두번째 짜리 까지 계산 )
# print(result3) # 종가 이동평균
print(round(result4, 2)) # 종가 이동평균 일간 변화율 ( 소수점 두번째 짜리 까지 계산 )

print(result5) # 종가가 연속으로 상승 or 하락 or X

# csv파일(stock_history_added.csv)로 저장
